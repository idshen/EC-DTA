    ...
    public class CnCNetTunnel
    {
        ...
        public static CnCNetTunnel Parse(string str)
        {
            ...
            catch (Exception ex)
            {
                ...
                {
                    Logger.Log("解析网络信息失败: " + ex.Message + Environment.NewLine + "解析字符串: " + str);
                    return null;
                }
        ...
        public List<int> GetPlayerPortInfo(int playerCount)
        {
            try
            {
                Logger.Log($"正在连接网络 {Address}:{Port}");
            ...
            {
                Logger.Log("无法连接到指定的网络服务器。返回的错误消息: " + ex.Message);
            }
        ...
        public void UpdatePing()
        {
            using (Ping p = new Ping())
            {
                ...
                {
                    Logger.Log($"在 pinging {Name} 网络服务器时捕获异常: {ex.Message}");
                }
            }
        }
    }
